...
glm::vec3 UniformGridVolume::GetGradientCentral(const glm::vec3& coord) const
{
    vec3 c = coord;
    ivec3 res = GetRes();
    ivec3 c_low = glm::floor(c * vec3(res - 1));
    float c_intensity = GetVec4(data_.data(), Index(c_low, res) * data_dim_, data_dim_).a;
    float delta_x = GetVec4(data_.data(), Index(c_low + ivec3(1, 0, 0), res) * data_dim_, data_dim_).a - c_intensity;
    float delta_y = GetVec4(data_.data(), Index(c_low + ivec3(0, 1, 0), res) * data_dim_, data_dim_).a - c_intensity;
    float delta_z = GetVec4(data_.data(), Index(c_low + ivec3(0, 0, 1), res) * data_dim_, data_dim_).a - c_intensity;

    // TODO: Or me!
    return vec3(delta_x, delta_y, delta_z) * vec3(res - 1);
}
